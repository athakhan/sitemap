...
def build_tree(rows)
  ...
  rows.each do |row|
    ...
    node.children << Node.new("Articulos Hub", "landing") if row["articles"].match(/yes/i)
    node.children << Node.new("Gallerias Hub", "landing") if row["galleries"].match(/yes/i)
    ...
    node.children << Node.new("Resumenes Hub", "landing") if row["recaps"].match(/yes/i)
    node.children << Node.new("Capitulos Hub", "landing") if row["episodes"].match(/yes/i)
